...
def training_loop(dataloader_X, dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        if opts.use_diffaug:
            images_X = diffaug_transforms(images_X)
            images_Y = diffaug_transforms(images_Y)

        # 1. Compute the discriminator losses on real images
        D_X_loss = torch.mean(((D_X(images_X) - 1)**2).flatten())
        D_Y_loss = torch.mean(((D_Y(images_Y) - 1)**2).flatten())
        ...
        D_X_loss = torch.mean(((D_X(fake_X.detach())) ** 2).flatten())

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = torch.mean(((D_Y(fake_Y.detach())) ** 2).flatten())
        ...
        if opts.use_diffaug:
            fake_X = diffaug_transforms(fake_X)

        # 2. Compute the generator loss based on domain X
        g_loss = torch.mean(((D_X(fake_X) - 1) ** 2).flatten())
        logger.add_scalar('G/XY/fake', g_loss, iteration)

        if opts.use_cycle_consistency_loss:
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            # image_Y_flat = images_Y.flatten(start_dim=1)
            # recon_Y = G_XtoY(G_YtoX(images_Y)).flatten(start_dim=1)
            # norm_Y = torch.linalg.norm(image_Y_flat - recon_Y, ord=1, dim=1)
            # cycle_consistency_loss = torch.mean(norm_Y)

            recon_Y = G_XtoY(G_YtoX(images_Y))
            cycle_consistency_loss = cycle_criterion(recon_Y, images_Y)
        ...
        if opts.use_diffaug:
            fake_Y = diffaug_transforms(fake_Y)

        # 2. Compute the generator loss based on domain Y
        g_loss += torch.mean(((D_Y(fake_Y) - 1) ** 2).flatten())
        logger.add_scalar('G/YX/fake', g_loss, iteration)

        if opts.use_cycle_consistency_loss:
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            # image_X_flat = images_X.flatten(start_dim=1)
            # recon_X = G_YtoX(G_XtoY(images_X)).flatten(start_dim=1)
            # norm_X = torch.linalg.norm(image_X_flat - recon_X, ord=1, dim=1)
            # cycle_consistency_loss = torch.mean(norm_X)

            recon_X = G_YtoX(G_XtoY(images_X))
            cycle_consistency_loss = cycle_criterion(recon_X, images_X)
...
if __name__ == '__main__':
    ...
    # change sample dir for colab
    opts.sample_dir = os.path.join(opts.sample_dir,
                                   '%s_%g' % (opts.X.split('/')[0], opts.lambda_cycle))
