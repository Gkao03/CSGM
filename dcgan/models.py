...
class DCGANGenerator(nn.Module):
    ...
    """
    def __init__(self, z_dim, ngf, channels_img):
        """
        ...
        channel_sizes = [ngf * 2 ** x for x in reversed(range(4))]  # [512, 256, 128, 64] -> 4 conv blocks
        ...
        for i, channel_size in enumerate(channel_sizes[1:]):  # other conv blocks
            self.main.add_module(f'block{i + 2}', self._block(channel_size * 2, channel_size, 4, 2, 1))
...
class DCGANDiscriminator(nn.Module):
    ...
    """
    def __init__(self, ndf, channels_img):
        """
        ...
        self.main = nn.Sequential()
        channel_sizes = [ndf * 2 ** x for x in range(4)]  # [64, 128, 256, 512] -> 4 conv blocks

        self.main.add_module('block1', self._block(channels_img, channel_sizes[0], 4, 2, 1, True))  # first conv block

        for i, channel_size in enumerate(channel_sizes[1:]):  # other conv blocks
            self.main.add_module(f'block{i + 2}', self._block(channel_size // 2, channel_size, 4, 2, 1, False))

        # last convolution
        self.main.add_module('lastConv', nn.Conv2d(in_channels=channel_sizes[-1], out_channels=1, kernel_size=4, stride=1, padding=0, bias=False))
